...
def useGemini(csv_url, question):
    ...
    agent = create_csv_agent(GoogleGenerativeAI(temprature=0, model="gemini-1.5-flash"), temp_file_path, verbose=True, allow_dangerous_code=True)
    final = agent.run(question)

    messages = [
        ("system", "Use this data to generate a vega-lite json code, no other reply."),
        ("human", final),
    ]
